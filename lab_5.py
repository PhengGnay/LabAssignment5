...
Then from the text file, we create a list of strings from each line on the text file.
Then, we create a dictionary the list.
Once the text file has been read into memory, we close the file.

We then decline a functions for translating which involves splitting the user input (sentence) into an
...
After each word is rtranslated, we then
...
main():
    set sentence = input()
    set dictionary = create_dictionay()
